...
// The path is by default absolute (not relative to wrk_dir_) !
...
//  in_file: Input file
//  in_dir_type: Relative to wrk_dir_ or absolute?
void CompactStar::CompOSE_EOS::ImportEOS(
	const Zaki::String::Directory &in_file,
	const bool &gen_plots)
{
	// std::cout << "\n\t " << wrk_dir_ + "/" + in_file << "\n" ;
	eos.Import(wrk_dir_ + "/" + in_file + ".eos");
	m_eff.Import(wrk_dir_ + "/" + in_file + "_m_eff.micro");
	V_eff.Import(wrk_dir_ + "/" + in_file + "_V.micro");
	U.Import(wrk_dir_ + "/" + in_file + "_U.micro");
}
	...
	const Dir_Type in_dir_type) const
{
	...
	fermi_ds.Export(file_dir + "/" + name_ + "_E_Fermi.tsv", {}, '\t');
}
