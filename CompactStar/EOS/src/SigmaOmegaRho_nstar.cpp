...
#include "CompactStar/EOS/SigmaOmegaRho_nstar.hpp"
...
//==============================================================
// Constructor
CompactStar::SigmaOmegaRho_nstar::SigmaOmegaRho_nstar() 
: Model({0.01, 10*0.153 }),
  ...
  neutron     (9,  0, NEUTRON_M_FM, -1./2., 1, 1, 1),
  neutron_star     (11,  0, NEUTRON_M_FM, -1./2., 1, 1, 1),
  electron    (10, -1, ELECTRON_M_FM)
{
  proton.SetName("Proton") ;
  neutron.SetName("Neutron");
  neutron_star.SetName("Neutron*");
  ...
  neutron_star.inclusion     = valid_rho    ;
...
//--------------------------------------------------------------
void CompactStar::SigmaOmegaRho_nstar::SetPars(const SigmaOmegaPar& in_pars)
{
  params = in_pars ;
...
//--------------------------------------------------------------
void CompactStar::SigmaOmegaRho_nstar::SetXRhos(const gsl_vector* x)
{
...
//--------------------------------------------------------------
int CompactStar::SigmaOmegaRho_nstar::Eq(const gsl_vector* x, gsl_vector* in_f)
{
  // g_sigma * sigma
...
int CompactStar::SigmaOmegaRho_nstar::SolveEq()
{
  ...
  Zaki::Math::GSLMultiFWrapper<SigmaOmegaRho_nstar, 
    int (SigmaOmegaRho_nstar::*)(const gsl_vector*, gsl_vector*)> 
  func(this, &SigmaOmegaRho_nstar::Eq, eq_size) ;
...
//--------------------------------------------------------------
// Prints the status
void CompactStar::SigmaOmegaRho_nstar::PrintState(const size_t& iter, 
...
//--------------------------------------------------------------
/// Jacobian
int CompactStar::SigmaOmegaRho_nstar::Jacob(const gsl_vector* x, gsl_matrix* J)
{
  const double gsig = gsl_vector_get(x,0);
...
//--------------------------------------------------------------
int CompactStar::SigmaOmegaRho_nstar::EqFdf(const gsl_vector* x, gsl_vector* in_f, gsl_matrix* J) 
{
  const double gsig = gsl_vector_get(x,0);
  ...
  proton.SetRho(xp * rho) ;
  // sigma_minus.SetRho(xsig_minus * rho) ;
  neutron.SetRho((1 - xp - xsig_minus) * rho / 2.0) ;
  neutron_star.SetRho((1 - xp - xsig_minus) * rho / 2.0) ;
...
int CompactStar::SigmaOmegaRho_nstar::SolveEqJacob()
{
  ...
  Zaki::Math::GSLMultiFdfWrapper<SigmaOmegaRho_nstar, 
    int (SigmaOmegaRho_nstar::*)(const gsl_vector*, gsl_vector*), 
    int (SigmaOmegaRho_nstar::*)(const gsl_vector*, gsl_matrix*),
    int (SigmaOmegaRho_nstar::*)(const gsl_vector*, gsl_vector*, gsl_matrix*)
    > 
    fdf(this, &SigmaOmegaRho_nstar::Eq, &SigmaOmegaRho_nstar::Jacob, 
      &SigmaOmegaRho_nstar::EqFdf, eq_size) ;
...
//--------------------------------------------------------------
// Energy density
double CompactStar::SigmaOmegaRho_nstar::EDens(const double& in_rho) 
{
  SetRho(in_rho) ;
  UpdatePool(in_rho) ;
  SolveEq() ;
...
//--------------------------------------------------------------
// Pressure
double CompactStar::SigmaOmegaRho_nstar::Press(const double& in_rho) 
{
  SetRho(in_rho) ;
  UpdatePool(in_rho) ;
  SolveEq() ;
...
void CompactStar::SigmaOmegaRho_nstar::UpdatePool(const double& in_rho) 
{
  Particle::EmptyPool() ;
...
std::vector<double> CompactStar::SigmaOmegaRho_nstar::EOSRow(const double& rho_i) 
{
  return {EDens(rho_i), Press(rho_i), rho_i, 
          proton.Rho/rho_i, neutron.Rho/rho_i, neutron_star.Rho/rho_i, 
...
std::string CompactStar::SigmaOmegaRho_nstar::EOSHeader() const
{
  char tmp_header[500] ;
  snprintf(tmp_header, sizeof(tmp_header), 
                      ...
                      "\t %-14s\t %-14s\t %-14s"
                      , 
            "e(g/cm^3)", "p(dyne/cm^2)", "rho(1/fm^3)", "r_p",
            "r_n", "r_sig-", "r_lam0", "r_sig0",
            "r_sig+", "r_xi-", "r_xi0", "r_e" , "r_mu",
            "Mu_p(MeV)", "Mu_n(MeV)", "Mu_e(MeV)", "Mu_mu(MeV)",
            "d_Mu_nn(fm^2)", "d_Mu_np(fm^2)", "d_Mu_ne(fm^2)", 
            "d_Mu_nmu(fm^2)", "d_Mu_e(fm^2)", "d_Mu_mu(fm^2)") ;
  return tmp_header ;
}
//--------------------------------------------------------------
/// Contains the information for a good guess depending on rho
void CompactStar::SigmaOmegaRho_nstar::FindGuess(const double& in_rho) 
{
  ...
  // Lambda0 enters
    ...
    root_guess = {0.715*MN, 0.224, 6e-4, 0.216, 3.4e-1} ;
...
void CompactStar::SigmaOmegaRho_nstar::SetEquilibriumEqs(const double& in_rho) 
{
