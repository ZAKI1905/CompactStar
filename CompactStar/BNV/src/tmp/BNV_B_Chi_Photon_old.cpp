...
{ 
  ...
  result *= C_2 ;
  result *= pow(m, 4) / (2*M_PI*M_PI*M_PI)  ;
...
void BNV_B_Chi_Photon::Plot_DecayLimits() 
{
  ...
  dec_lim_ds.Reserve(5, m_chi_res) ;
  ...
  for (size_t i = 0; i < m_chi_res; i++)
  {
    ...
    //                  Neutron 
    // ------------------------------------------
    double dec_result = EvalDecayRate(m_chi, neutron) ;
    // if (dec_result.max == 0)
    ...
    double eps_lim = 1e-10*sqrt(7.83129*1e-10 / dec_result ) ;
    ...
    dec_lim_ds[1].vals.emplace_back( eps_lim );
    ...
    dec_lim_ds[2].vals.emplace_back( Vacuum_Decay_Br(neutron, m_chi, eps_lim)
                                    );
    // ------------------------------------------
    //                  Lambda 
    // ------------------------------------------
    ...
    // if (dec_result.max == 0)
    //   dec_result.max = 1e-60 ;
    eps_lim = 1e-10*sqrt(7.83129*1e-10 / dec_result ) ;
    ...
    dec_lim_ds[3].vals.emplace_back( eps_lim );
    ...
    dec_lim_ds[4].vals.emplace_back( Vacuum_Decay_Br(lambda, m_chi, eps_lim)
                                    );
  }
  
  dec_lim_ds[0].label = "$m_{\\chi}\\, [ MeV ]$" ;
  dec_lim_ds[1].label = "$\\varepsilon_{n}\\, [ MeV ]$" ;
  dec_lim_ds[2].label = "Br$(n \\to \\chi \\gamma)$" ;

  // dec_lim_ds[2].label = "$\\varepsilon_{n}^{nor}\\, [ MeV ]$" ;
  // dec_lim_ds[3].label = "$\\varepsilon_{n}^{max}\\, [ MeV ]$" ;
  dec_lim_ds[3].label = "$\\varepsilon_{\\Lambda}\\, [ MeV ]$" ;
  // dec_lim_ds[5].label = "$\\varepsilon_{\\Lambda}^{nor}\\, [ MeV ]$" ;
  // dec_lim_ds[6].label = "$\\varepsilon_{\\Lambda}^{max}\\, [ MeV ]$" ;
  dec_lim_ds[4].label = "Br$(\\Lambda \\to \\chi \\gamma)$" ;

  dec_lim_ds.SetWrkDir(wrk_dir + "/BNV_2023/results/B_Chi_Photon_Decay/"+model) ;
  ...
  dec_lim_ds.SemiLogYPlot(0, {{1, "$\\varepsilon_{n}$"}, 
          {3, "$\\varepsilon_{\\Lambda}$"}}, "Eps_vs_m_chi.pdf",
           pulsar.GetName() + "\n" + model) ;
  ...
  dec_lim_ds.SemiLogYPlot(0, {2,4}, "Br_vs_m_chi.pdf",
                          pulsar.GetName() + "\n" + model) ;
}
