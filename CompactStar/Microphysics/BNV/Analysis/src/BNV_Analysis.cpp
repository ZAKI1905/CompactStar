...
#include "CompactStar/Microphysics/BNV/Analysis/BNV_Analysis.hpp"
...
void MicroBNVAna::BNV_Analysis::Analyze(Core::NStar *in_star)
{
	...
	for (size_t i = 0; i < bar_list.size(); i++)
	{
		...
		Zaki::Vector::DataColumn b_den = *in_star->GetRho_i(bar_list[i].label) *
										 *in_star->Profile().GetBaryonDensity();
...
//--------------------------------------------------------------
// Saves the results
void MicroBNVAna::BNV_Analysis::Export(const Zaki::String::Directory &in_dir)
{
...
//--------------------------------------------------------------
void MicroBNVAna::BNV_Analysis::Evolve(const Zaki::String::Directory &in_dir)
{
...
// void MicroBNVAna::BNV_Analysis::ExportBNV(const Zaki::String::Directory& in_dir)
// {
// ---------------------------------------------
//              Neutron
// ---------------------------------------------
