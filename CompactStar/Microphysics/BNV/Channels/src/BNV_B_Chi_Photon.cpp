...
#include "CompactStar/Microphysics/BNV/Channels/BNV_B_Chi_Photon.hpp"
...
{
	return 5 * x_5_coeff * pow(x, 4) + 7 * x_7_coeff * pow(x, 6) - 2.42 * x_242_coeff * pow(x, 1.42);
}
...
//==============================================================
// Constructor
MicroBNVCh::BNV_B_Chi_Photon::BNV_B_Chi_Photon()
...
MicroBNVCh::BNV_B_Chi_Photon::~BNV_B_Chi_Photon() {}
//--------------------------------------------------------------
MicroBNVInt::BNV_Chi::Process MicroBNVCh::BNV_B_Chi_Photon::GetSpecificProcess(const Baryon &B) const
{
...
double MicroBNVCh::BNV_B_Chi_Photon::Cos_Theta_CMpChi_NMpB(const double &E_chi_NM_over_mB, const double &mu_chi,
...
// double MicroBNVCh::BNV_B_Chi_Photon::Cos_Theta_CMpChi_NMpB_Escape(const double& mu_chi,
//                                                const double& x, const double& sigma_0)
// {
//   int idx = 0 ;
//   double nu = *pulsar.GetProfile()->GetMetricNu()[idx] ;
...
//                  m^4 C / (pi^2)
//
double MicroBNVCh::BNV_B_Chi_Photon::Escape_PhaseSpace_Integrand(const double &x)
{
	...
	isotropic *= (1 + sigma_0 * sigma_0 + 2 * x * sigma_0) * (1 + x * sigma_0 + 2 * mu_chi) + pow(mu_chi, 2) * (1 + x * sigma_0);
...
double MicroBNVCh::BNV_B_Chi_Photon::Escape_PhaseSpace_Integral(const Baryon &B,
																const double &m,
																const double &m_chi,
																...
																const double &x_F)
{
	...
	Zaki::Math::GSLFuncWrapper<BNV_B_Chi_Photon, double (MicroBNVCh::BNV_B_Chi_Photon::*)(const double &)>
		Fp(this, &MicroBNVCh::BNV_B_Chi_Photon::Escape_PhaseSpace_Integrand);
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Escape_Rate_vs_R(const Baryon &B,
																	 const double &m_chi,
																	 const bool &export_flag)
{
	Zaki::Vector::DataColumn kF_2 = (3 * M_PI * M_PI * micro_r["n_" + B.label]).pow(2.0 / 3.0);
	...
	Zaki::Vector::DataColumn nu_r = *pulsar.GetProfile()->GetMetricNu();
	...
	if (export_flag)
	{
		...
		rate_vs_r.AddHead("# Rates of chi-escape due to' " + process.name +
						  " from " + pulsar.GetName() + ", and assuming m_chi = " + std::string(tmp_mchi) + " MeV.\n");

		rate_vs_r.Export("Escape_Rate_vs_R_" + model + "_" + pulsar.GetName() + "_" + B.label + "_m=" + tmp_mchi + ".tsv");
	}
	// ...........................................
...
double MicroBNVCh::BNV_B_Chi_Photon::Escape_Ratio_Total(const Baryon &B,
														const double &m_chi,
														const bool &export_flag)
{
	Zaki::Vector::DataColumn r = *pulsar.GetProfile()->GetRadius();
	Zaki::Vector::DataColumn M_r = *pulsar.GetProfile()->GetMass();
	Zaki::Vector::DataColumn nu_r = *pulsar.GetProfile()->GetMetricNu();
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Escape_Ratio_Total(const Baryon &B,
																	   ...
																	   const bool &export_flag)
{
	...
	if (export_flag)
	{
		...
		ds_escape_ratio_vs_mchi.AddHead("# Rates of chi-escape due to '" + process.name +
										" from " + pulsar.GetName() + ", as a function of m_chi in the range [ " + std::string(tmp_mchi_min) + ", " + std::string(tmp_mchi_max) + "] MeV.\n");

		ds_escape_ratio_vs_mchi.Export("Escape_Ratio_vs_mchi_" + model + "_" + pulsar.GetName() + "_" + B.label + ".tsv");
	}
	// ...........................................
...
void MicroBNVCh::BNV_B_Chi_Photon::UseCGSUnits()
{
	cgs_units = true;
}
//--------------------------------------------------------------
...
// Sets the EoS model
// void MicroBNVCh::BNV_B_Chi_Photon::SetModel(const std::string& in_eos_model)
// {
//   model = in_eos_model ;
// }

//--------------------------------------------------------------
// void MicroBNVCh::BNV_B_Chi_Photon::GenSequence() const
// {
...
//   Zaki::Vector::DataSet tmp_seq(wrk_dir_ +
//                   "/BNV_2022/results/B_Chi_Photon_Decay/"+ model,
...
//    Also mu_chi --> mu.
double MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_PhaseSpace_Integrand(const double &x)
{
...
double MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_PhaseSpace_Integrand(const double &x)
{
...
// [Apr 15, 2023]: The old formula which was incorrect is now
//                  replaced with the new one.
double MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_PhaseSpace_Integral(
	const Baryon &B,
	const double &m,
	const double &m_chi,
	const double &Sigma_0,
	const double &x_F)
{
	...
	Zaki::Math::GSLFuncWrapper<BNV_B_Chi_Photon, double (MicroBNVCh::BNV_B_Chi_Photon::*)(const double &)>
		Fp(this, &MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_PhaseSpace_Integrand);
	...
	double C = pow(B.g * default_eps * q_e, 2) * pow(m, 3) / (768 * M_PI * M_PI * M_PI);
...
double MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_PhaseSpace_Integral(const Baryon &B,
																	  const double &m,
																	  const double &m_chi,
																	  const double &Sigma_0,
																	  const double &x_F)
{
	...
	Zaki::Math::GSLFuncWrapper<BNV_B_Chi_Photon, double (MicroBNVCh::BNV_B_Chi_Photon::*)(const double &)>
		Fp(this, &MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_PhaseSpace_Integrand);
	...
	// Factor behind the integral
	double C = pow(B.g * default_eps * q_e, 2) * pow(m, 3) / (128 * M_PI * M_PI * M_PI);
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_E_Rate_vs_Density(
	const double &m_chi,
	const Baryon &B,
...
void MicroBNVCh::BNV_B_Chi_Photon::hidden_Plot_Thermal_Hole_E_Rate_vs_Density(
	const Baryon &B,
	const double &m_chi,
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_E_Rate_vs_R(const double &m_chi,
																			 const Baryon &B,
																			 const bool &gen_plots)
{
	...
	Zaki::Vector::DataColumn n_r = *pulsar.GetProfile()->GetBaryonDensity();
...
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_E_Rate_vs_R(const double &m_chi)
{
...
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_E_Rate_vs_R(const std::vector<double> &m_chi)
{
...
/// for a set of chi's in the same figure
/// as a function of radius in units of ?
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Hole_E_Rate_vs_R(const std::vector<double> &m_chi, const Baryon &B)
{
	Zaki::Vector::DataSet rate_vs_r(*pulsar.GetProfile()->GetRadius());
	...
	rate_vs_r.AddHead("# Rates of thermal energy dumped into the "
					  ...
					  GetSpecificProcess(B).name + "' assuming eps = " + std::string(tmp_eps) + " MeV as a function of radius in '" + pulsar.GetName() + "'.\n" + "# The unit for rates is in " + unit_str + ".\n");

	rate_vs_r.Export("Thermal_Hole_Rate_vs_R_" + std::string(B.label.c_str()) + "_" + model + "_" + pulsar.GetName() + ".tsv");
	// ------------------------------------
}
...
void MicroBNVCh::BNV_B_Chi_Photon::hidden_Plot_Thermal_Hole_E_Rate_vs_R(
	const Baryon &B,
	const double &m_chi,
...
//--------------------------------------------------------------
void MicroBNVCh::BNV_B_Chi_Photon::hidden_Plot_Thermal_Photon_E_Rate_vs_R(
	const Baryon &B,
	const double &m_chi,
	Zaki::Vector::DataSet *ds) const
{
...
//--------------------------------------------------------------
void MicroBNVCh::BNV_B_Chi_Photon::hidden_Plot_Thermal_Photon_E_Rate_vs_Density(const Baryon &B,
																				const double &m_chi,
																				Zaki::Vector::DataSet *ds)
{
	Zaki::String::Directory out_dir = wrk_dir_ + model + "/" + pulsar.GetName() + "/" + process.name + "/Rate_vs_Den/" + B.short_name;
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_E_Rate_vs_Density(
	const double &m_chi,
	const Baryon &B,
	const bool &gen_plots)
{
...
// Returns the rate of thermal energy dumped into the NS
//  per unit volume as a function of radius in units of MeV s^-1/fm^3
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_E_Rate_vs_R(const double &m_chi,
																			   const Baryon &B,
																			   const bool &gen_plots)
{
...
/// Plots rate of thermal energy dumped into the NS
/// as a function of radius in units of s^-1/fm^3
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_E_Rate_vs_R(const double &m_chi)
{
...
/// for a set of chi's in the same figure
/// as a function of radius in units of s^-1/fm^3
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Photon_E_Rate_vs_R(const std::vector<double> &m_chi)
{
															  ...
															  const Baryon &B)
{
	...
	rate_vs_r.Export("Thermal_Photon_Rate_vs_R_" + std::string(B.label.c_str()) + "_" + model + "_" + pulsar.GetName() + ".tsv");
	// ------------------------------------
}
...
double MicroBNVCh::BNV_B_Chi_Photon::GetThermal_Photon_E_Rate(
	const double &m_chi,
	const Baryon &B)
{
	Zaki::Vector::DataColumn r = *pulsar.GetProfile()->GetRadius();
...
void MicroBNVCh::BNV_B_Chi_Photon::Plot_Thermal_Photon_E_Rate()
{
	// ...........................................
	// Columns are:
	...
	rate.AddHead("# Total rates of thermal energy dumped into '" + pulsar.GetName() + "' in units of '" + unit_str + "' per baryon for '" + process.name + "' assuming eps = " + std::string(tmp_eps) + " MeV.\n");
...
void MicroBNVCh::BNV_B_Chi_Photon::Plot_Limited_Thermal_E_Rate() const
{
	...
	Zaki::Vector::DataSet default_rate(wrk_dir_ + model + "/" + pulsar.GetName() + "/" + process.name,
	...
	Zaki::Vector::DataSet eps_limits(wrk_dir_ + model + "/" + pulsar.GetName() + "/" + "Combo",
	...
	default_rate.AddHead("# Total rates of thermal energy dumped into '" + pulsar.GetName() + "' per baryon for '" + process.name + "' assuming the limits on eps " + "from PSR binary decay rates." + ".\n");
...
//
double MicroBNVCh::BNV_B_Chi_Photon::PhaseSpace_Integrand(const double &x)
{
	double mu_chi = phase_int_mu_chi;
	double sigma_0 = phase_int_sigma_0;
	...
	out *= (1 + sigma_0 * sigma_0 + 2 * x * sigma_0) * (1 + x * sigma_0 + 2 * mu_chi) + pow(mu_chi, 2) * (1 + x * sigma_0);
...
double MicroBNVCh::BNV_B_Chi_Photon::PhaseSpace_Integral(const Baryon &B,
														 const double &m,
														 const double &m_chi,
														 const double &Sigma_0,
														 const double &x_F)
{
	...
	Zaki::Math::GSLFuncWrapper<BNV_B_Chi_Photon, double (MicroBNVCh::BNV_B_Chi_Photon::*)(const double &)>
		Fp(this, &MicroBNVCh::BNV_B_Chi_Photon::PhaseSpace_Integrand);
...
// double MicroBNVCh::BNV_B_Chi_Photon::Resonance_B_Chi_Photon_Rate(const double& m_chi,
//                                                      const Baryon& in_B,
...
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Rate_vs_Density(
	const double &m_chi,
	const Baryon &B,
	const bool &gen_plots)
{
...
/// Returns rate
///  as a function of radius in units of s^-1/fm^3
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Rate_vs_R(const double &m_chi,
															  const Baryon &B,
															  const bool &gen_plots)
{
...
// Returns rate
//  as a function of radius in units of s^-1/fm^3
Zaki::Vector::DataSet MicroBNVCh::BNV_B_Chi_Photon::Rate_vs_R_Slow(const double &m_chi,
																   const Baryon &B,
																   const bool &gen_plots)
{
	Zaki::Vector::DataColumn kF_2 = (3 * M_PI * M_PI * micro_r["n_" + B.label]).pow(2.0 / 3.0);
...
// for a set of chi's in the same figure
// as a function of radius in units of s^-1/fm^3
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Total_E_Rate_vs_R(const std::vector<double> &m_chi)
{
...
// for a set of chi's in the same figure
// as a function of radius in units of ?
void MicroBNVCh::BNV_B_Chi_Photon::Thermal_Total_E_Rate_vs_R(
	const std::vector<double> &m_chi, const Baryon &B)
{
	...
	rate_vs_r.Export("Thermal_Total_Rate_vs_R_" + std::string(B.label.c_str()) + "_" + model + "_" + pulsar.GetName() + ".tsv");
	// ------------------------------------
}
...
// due to Fermi-hole heating
// integrated over the radius and in units of [ MeV or erg per sec ]
double MicroBNVCh::BNV_B_Chi_Photon::GetThermal_Hole_E_Rate(const double &m_chi, const Baryon &B)
{
	Zaki::Vector::DataColumn r = *pulsar.GetProfile()->GetRadius();
	Zaki::Vector::DataColumn M_r = *pulsar.GetProfile()->GetMass();
	Zaki::Vector::DataColumn nu_r = *pulsar.GetProfile()->GetMetricNu();
...
// Plots the total rate of thermal energy dumped into the NS
// as a function of m_chi
void MicroBNVCh::BNV_B_Chi_Photon::Plot_Thermal_Total_E_Rate()
{
	// ...........................................
	// Columns are:
	// rate[0] = m_chi, rate[1] = Gamma_n,
	...
	for (size_t i = 0; i < m_chi_vals.res; i++)
	{
		...
		double rate_n = GetThermal_Photon_E_Rate(m_chi, neutron) + GetThermal_Hole_E_Rate(m_chi, neutron);
		double rate_lam = GetThermal_Photon_E_Rate(m_chi, lambda) + GetThermal_Hole_E_Rate(m_chi, lambda);
...
void MicroBNVCh::BNV_B_Chi_Photon::Limit_eps_From_Heating(const double &T_core)
{
	Z_LOG_ERROR("Function under development!");
	...
	// double L_cool = pulsar.Get_Neutrino_Lumin() + pulsar.Get_Surface_Photon_Lumin();
	// // ...........................................
	// // Columns are:
	...
	// 	double L_BNV_n = GetThermal_Photon_E_Rate(m_chi, neutron) + GetThermal_Hole_E_Rate(m_chi, neutron);
	// 	// Remember to multiply by B
	// 	L_BNV_n *= pulsar.GetSeqPoint().b;

	// 	double L_BNV_lam = GetThermal_Photon_E_Rate(m_chi, lambda) + GetThermal_Hole_E_Rate(m_chi, lambda);
	// 	// Remember to multiply by B
	...
	// limits.AddHead("# The mixing values corresponding to a steady-sate at T_core = '" + std::string(T_core_str) +
	// 			   +"' kelvin, i.e., apparent (red-shifted) "
	// 				"surface temperature T_s = '" +
	// 			   std::string(T_surf_str) +
	// 			   "' kelvin for '" + pulsar.GetName() + "' in units of MeV "
...
//  assuming a core temperature T_core (in kelvin)
void MicroBNVCh::BNV_B_Chi_Photon::Limit_eps_From_Slowness(const double &T_core)
{
	Z_LOG_ERROR("Function under development!");
	...
	// int r_durca_idx = *pulsar.GetProfile()->GetRadius().GetClosestIdx(r_durca);
	...
	// limits.Export("Limi_eps_slow_" + std::string(T_core_fname_str) + "_" + model + "_" + pulsar.GetName() + ".tsv");
}

//--------------------------------------------------------------
// Solves for the mixing parameter eps given the beta-imbalance
// beta-imbalance: x = eta / T
void MicroBNVCh::BNV_B_Chi_Photon::Eps_From_Beta_Imbalance(const double &x)
{
	Z_LOG_ERROR("Function under development!");
	...
	// // Unit = [ erg s^-1 ]
	// double bnv_therm_n = GetThermal_Photon_E_Rate(m_chi, neutron) + GetThermal_Hole_E_Rate(m_chi, neutron);
	// // Remember to multiply by B
