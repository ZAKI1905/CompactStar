...
//==============================================================
// Constructor
MicroBNVInt::BNV_Chi::BNV_Chi(const Process &in_process)
...
//--------------------------------------------------------------
// Destructor
MicroBNVInt::BNV_Chi::~BNV_Chi() {}
//--------------------------------------------------------------
// Outputs the vacuum branching ratio of
//      B -> chi + photon
// 'in_m_chi' & 'in_eps_lam' must be in MeV
// ____________________________________
double MicroBNVInt::BNV_Chi::Vacuum_Decay_Br(const Baryon &B,
...
//--------------------------------------------------------------
// Sets the EoS model
void MicroBNVInt::BNV_Chi::SetModel(const std::string &in_eos_model)
{
	model = in_eos_model;
}
...
//--------------------------------------------------------------
// Sets m_chi_vals
void MicroBNVInt::BNV_Chi::SetChiMassRange(const Zaki::Math::Axis &in_m_chis)
{
...
void MicroBNVInt::BNV_Chi::SetBNVLimit(const double &in_gamma)
{
...
//--------------------------------------------------------------
// Imports the EoS
void MicroBNVInt::BNV_Chi::ImportEOS(const Zaki::String::Directory &eos_dir,
									 const std::string &micro_model)
{
	// Zaki::Vector::DataSet ds_EOS(dir.ParentDir().ParentDir() +"/EOS/CompOSE/DS(CMF)-"+ model,
	//                             "DS(CMF)-" + model + ".eos") ;

	// eos.SetWrkDir(wrk_dir_ + "/eos/CompOSE/" + model) ;
...
void MicroBNVInt::BNV_Chi::GenSequence() const
{
	...
	((Zaki::String::Directory)wrk_dir_ + "/" + model).Create();

	solver.Solve({{e_c_min, e_c_max}, 100, "Log"},
				 model + "/" + pulsar.GetName(),
				 model);

	Zaki::Vector::DataSet tmp_seq(wrk_dir_ + model + "/" + pulsar.GetName(),
								  model + "_Sequence.tsv");
	// tmp_seq[1].MaxIdx()
...
//--------------------------------------------------------------
// Attaches the pulsar
void MicroBNVInt::BNV_Chi::FindPulsar(const bool &gen_plots)
{
	bool has_hyperons = true;
	pulsar.SetWrkDir(wrk_dir_ + "/" + model + "/" + pulsar.GetName());

	std::string file_path = (wrk_dir_ + "/" + model + "/" + pulsar.GetName() + ".tsv").Str();
	...
	{
		Z_LOG_WARNING("Plot generation is currently disabled inside MicroBNVInt::BNV_Chi::FindPulsar");
	}
...
//--------------------------------------------------------------
double MicroBNVInt::BNV_Chi::PhaseSpace_Integrand(const double &x)
{
	return 0;
}

//--------------------------------------------------------------
double MicroBNVInt::BNV_Chi::PhaseSpace_Integral(const Baryon &B,
												 const double &m,
												 const double &m_chi,
...
//   Zaki::Math::GSLFuncWrapper<BNV_Chi, double (MicroBNVInt::BNV_Chi::*)(const double&)>
//     Fp(this, &MicroBNVInt::BNV_Chi::PhaseSpace_Integrand);
...
void MicroBNVInt::BNV_Chi::Plot_Meff_Radius()
{
	...
	Zaki::Vector::DataColumn dc_neutron_meff_r =
		ds_meff_n.Evaluate(1, b_density);

	Zaki::Vector::DataColumn dc_lambda_meff_r =
		ds_meff_n.Evaluate(2, b_density);

	Zaki::Vector::DataColumn dc_proton_meff_r =
		ds_meff_n.Evaluate(3, b_density);

	Zaki::Vector::DataColumn dc_sigma_meff_r =
		ds_meff_n.Evaluate(4, b_density);

	Zaki::Vector::DataSet ds_meff_r({*pulsar.GetProfile()->GetRadius(),
...
void MicroBNVInt::BNV_Chi::Plot_RestEnergy_Radius()
{
	...
	Zaki::Vector::DataColumn dc_neutron_E_r =
		ds_E_n.Evaluate(1, b_density);

	Zaki::Vector::DataColumn dc_proton_E_r =
		ds_E_n.Evaluate(2, b_density);

	Zaki::Vector::DataColumn dc_lambda_E_r =
		ds_E_n.Evaluate(3, b_density);

	Zaki::Vector::DataColumn dc_sigma_m_E_r =
		ds_E_n.Evaluate(4, b_density);
	// ......................................

	// ......................................
	//  Combining the data-columns into a dataset
	Zaki::Vector::DataSet ds_E_r({*pulsar.GetProfile()->GetRadius(),
...
void MicroBNVInt::BNV_Chi::Plot_EF_Radius()
{

	// ---------------------------------------------------
	// Finding Fermi energy as a function of density
	// ---------------------------------------------------
	Zaki::Vector::DataColumn fermi_electron = (pow(Zaki::Physics::ELECTRON_M_FM, 2) + (3 * M_PI * M_PI * eos.GetEOS("0") * eos.GetEOS(2)).pow(2. / 3.)).sqrt() / Zaki::Physics::MEV_2_INV_FM;

	Zaki::Vector::DataColumn fermi_muon = (pow(Zaki::Physics::MUON_M_FM, 2) + (3 * M_PI * M_PI * eos.GetEOS("1") * eos.GetEOS(2)).pow(2. / 3.)).sqrt() / Zaki::Physics::MEV_2_INV_FM;

	Zaki::Vector::DataColumn fermi_neutron = (m_B_ds[neutron.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS("10") * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt() + sig0_ds[neutron.label];

	Zaki::Vector::DataColumn fermi_lambda = (m_B_ds[lambda.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS("100") * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt() + sig0_ds[lambda.label];

	Zaki::Vector::DataColumn fermi_proton = (m_B_ds[proton.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS("11") * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt() + sig0_ds[proton.label];
	...
	Zaki::Vector::DataColumn dc_electron_EF_r =
		fermi_ds_n.Evaluate(1, b_density);

	Zaki::Vector::DataColumn dc_muon_EF_r =
		fermi_ds_n.Evaluate(2, b_density);

	Zaki::Vector::DataColumn dc_neutron_EF_r =
		fermi_ds_n.Evaluate(3, b_density);

	Zaki::Vector::DataColumn dc_lambda_EF_r =
		fermi_ds_n.Evaluate(4, b_density);

	Zaki::Vector::DataColumn dc_proton_EF_r =
		fermi_ds_n.Evaluate(5, b_density);
	// ......................................

	// ......................................
	//  Combining the data-columns into a dataset
	Zaki::Vector::DataSet ds_EF_r({*pulsar.GetProfile()->GetRadius(),
...
void MicroBNVInt::BNV_Chi::Plot_Estar_Radius(const Baryon &B)
{
	// ---------------------------------------------------
	// Finding maximum E* as a function of density
	// ---------------------------------------------------
	Zaki::Vector::DataColumn Estar_max_n = (m_B_ds[B.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS(B.label) * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt();
	...
	Zaki::Vector::DataColumn dc_B_Estar_max_r =
		ds_Estar_max_n.Evaluate(1, b_density);
	// ......................................
	...
	Zaki::Vector::DataColumn dc_B_Estar_min_r =
		ds_Estar_min_n.Evaluate(1, b_density);
	// ......................................

	// ......................................
	//  Combining the data-columns into a dataset
	Zaki::Vector::DataSet ds_E0_EF_r({*pulsar.GetProfile()->GetRadius(),
...
void MicroBNVInt::BNV_Chi::Plot_CM_E_Radius(const Baryon &B)
{
	// ---------------------------------------------------
	...
	Zaki::Vector::DataColumn ECM_max_n = (m_B_ds[B.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS(B.label) * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt() * 2 * sig0_ds[B.label];
	...
	Zaki::Vector::DataColumn dc_B_ECM_max_r =
		ds_ECM_max_n.Evaluate(1, b_density);
	// ......................................
	...
	Zaki::Vector::DataColumn dc_B_ECM_min_r =
		ds_ECM_min_n.Evaluate(1, b_density);
	// ......................................
...
void MicroBNVInt::BNV_Chi::Plot_RestE_EF_Radius(const Baryon &B)
{

	// ---------------------------------------------------
	// Finding Fermi energy as a function of density
	// ---------------------------------------------------
	Zaki::Vector::DataColumn fermi_B = (m_B_ds[B.label].pow(2) + (3 * M_PI * M_PI * eos.GetEOS(B.label) * eos.GetEOS(2)).pow(2. / 3.) / pow(Zaki::Physics::MEV_2_INV_FM, 2)).sqrt() + sig0_ds[B.label];
...
// double
// MicroBNVInt::BNV_Chi::EvalDecayRate(const double& m_chi, const Baryon& in_B)
// {
//   std::cout << "\n -------------------- m_chi = " << m_chi
//             <<  " MeV ---- B = " << in_B.label <<"-----------------\n" ;
...
void MicroBNVInt::BNV_Chi::PlotVacuumBrLim(const Baryon &B)
{
...
//--------------------------------------------------------------
// Finds and plots the vacuum branching ratio limits
void MicroBNVInt::BNV_Chi::PlotVacuumBrLim()
{
...
// void MicroBNVInt::BNV_Chi::ImportEffMass()
// {
//   m_B_ds.SetWrkDir(wrk_dir_) ;
...
// void MicroBNVInt::BNV_Chi::ImportVSelfEnergy()
// {
...
// void MicroBNVInt::BNV_Chi::Plot_RestEnergy_Density()
// {
...
void MicroBNVInt::BNV_Chi::PlotRate_Eps(const Baryon &B)
{
	// Columns are:
	...
	rate.AddHead("# Rates of '" + process.name + "' assuming eps = " + std::string(tmp_eps) + " MeV.\n"
																							  "# Limits on eps are evaluated using Gamma_BNV < " +
				 std::string(tmp_gamma) + ", which itself is inferred from the" + " orbital decay rate of " + pulsar.GetName() + ".\n");
...
void MicroBNVInt::BNV_Chi::PlotRate_Eps()
{
...
// Zaki::Vector::DataSet MicroBNVInt::BNV_Chi::Rate_vs_R(
//                                 const double& m_chi,
//                                 const Baryon& B,
...
//   if(gen_plots)
//   {
//     Zaki::String::Directory out_dir = wrk_dir_ + model + "/"
...
//  as a function of radius in units of s^-1 fm^-3
void MicroBNVInt::BNV_Chi::Rate_vs_R(const double &m_chi)
{
...
// as a function of radius in units of s^-1 fm^-3
void MicroBNVInt::BNV_Chi::Rate_vs_R(const std::vector<double> &m_chi, const Baryon &B)
{
	Zaki::Vector::DataSet rate_vs_r(*pulsar.GetProfile()->GetRadius());
	...
	rate_vs_r.AddHead("# Rates per volume for '" + GetSpecificProcess(B).name + "' assuming eps = " + std::string(tmp_eps) + " MeV as a function of radius in '" + pulsar.GetName() + "'.\n" + "# The unit for rates is in " + "[ s^-1 / fm^3 ].\n");

	rate_vs_r.Export("Rate_vs_R_" + std::string(B.label.c_str()) + "_" + model + "_" + pulsar.GetName() + ".tsv");
	// ------------------------------------
}
...
//  [ Incomplete ]
void MicroBNVInt::BNV_Chi::Rate_vs_R(const std::vector<double> &m_chi,
									 const Baryon &B,
									 const std::vector<MicroBNVInt::Pulsar_Limit> &psr_lims)
{
...
//--------------------------------------------------------------
void MicroBNVInt::BNV_Chi::hidden_Plot_Rate_vs_R(const Baryon &B,
												 const double &m_chi,
...
//--------------------------------------------------------------
void MicroBNVInt::BNV_Chi::hidden_Plot_Rate_vs_Density(const Baryon &B,
													   const double &m_chi,
													   Zaki::Vector::DataSet *ds) const
{
	Zaki::String::Directory out_dir = wrk_dir_ + model + "/" + pulsar.GetName() + "/" + process.name + "/Rate_vs_Den/" + B.short_name;
...
// Zaki::Vector::DataSet MicroBNVInt::BNV_Chi::Rate_vs_Density(
//                                       const double& m_chi,
//                                       const Baryon& B,
//                                 const bool& gen_plots)
// {
...
// Zaki::Vector::DataColumn MicroBNVInt::BNV_Chi::SigmaPlus(
//                         const double& m_chi,
//                         const Baryon& B)
// {
...
// // The arguments are (m_chi, m_B^*, n [fm^-3] ).
// Zaki::Vector::DataColumn MicroBNVInt::BNV_Chi::SigmaMinus(
...
// // self-energy, from condition (a).
// // The arguments are (m_chi, m_B^*, n [fm^-3] ).
// double MicroBNVInt::BNV_Chi::SigmaMinus(
//                         const double& m_chi,
//                         const double& m_B,
//                         const double& n_B)
// {
...
// // self-energy, from condition (b).
// // The arguments are (m_chi, m_B^*, n [fm^-3] ).
// double MicroBNVInt::BNV_Chi::SigmaPlus(
//                   const double& m_chi,
//                   const double& m_B,
//                   const double& n_B)
// {
									 ...
									 const Baryon &B)
{
	...
	{
		Z_LOG_ERROR("BNV_Chi::GetRate: required columns (r,M,nu) not found.");
		return 0.0;
	}

	// make local working copies
	Zaki::Vector::DataColumn r = *r_col;	 // [km]
	Zaki::Vector::DataColumn M_r = *m_col;	 // [M_sun]
	Zaki::Vector::DataColumn nu_r = *nu_col; // dimensionless
	...
	// 4) proper-volume + redshift factor:
	//    4π r² / sqrt(1 - 2M/r) × e^{ν(r)}
	Zaki::Vector::DataColumn decay_integrand_dc = 4.0 * M_PI * r.pow(2);
	decay_integrand_dc *= dc_rate_r;
	decay_integrand_dc /= (1.0 - 2.0 * M_r / r).sqrt();
	decay_integrand_dc *= exp(nu_r);

	// 5) integrate over radius
	Zaki::Vector::DataSet decay_integral_ds({r, decay_integrand_dc});
	decay_integral_ds.Interpolate(0, 1);
	...
	double total_rate_sinv = decay_integral_ds.Integrate(1, {r_min, r_max}); // [s⁻¹]
	...
	if (Btot <= 0.0)
	{
		Z_LOG_ERROR("BNV_Chi::GetRate: seq point has zero/negative baryon number.");
		return 0.0;
	}

	const double per_baryon_rate_yrinv = total_rate_sinv / Btot;

	std::cout << "\n ---------------------------------------- ";
	std::cout << "\n\t Full decay rate (B_dot/B) : "
			  << per_baryon_rate_yrinv << " per year.\n";

	return per_baryon_rate_yrinv;
}
									   ...
									   const Baryon &B)
{
	...
	if (!r_col || !m_col || !nu_col)
	{
		Z_LOG_ERROR("MicroBNVInt::BNV_Chi::GetEpsLim: missing r/M/nu columns in profile.");
		return std::numeric_limits<double>::max();
	}
	...
	if (Btot <= 0.0)
	{
		Z_LOG_ERROR("MicroBNVInt::BNV_Chi::GetEpsLim: seq point has zero/negative baryon number.");
		return std::numeric_limits<double>::max();
	}

	const double rate_per_baryon_yrinv = total_rate_sinv / Btot;

	std::cout << "\n ---------------------------------------- ";
	std::cout << "\n\t Full decay rate (B_dot/B) : "
			  << rate_per_baryon_yrinv << " per year.\n";

	// 8) ε-limit
	// model: Γ_model(ε) = Γ_model(ε₀) (ε/ε₀)²  ⇒ ε_lim = ε₀ √(Γ_sd / Γ_model)
	if (rate_per_baryon_yrinv <= 0.0)
	{
		return std::numeric_limits<double>::max();
	}

	const double eps_lim = default_eps * std::sqrt(gamma_bnv_bin_lim / rate_per_baryon_yrinv);
	return eps_lim;
}
...
// //  B -> chi decay rate integrated over the radius
// //  and in units of [ yr^-1 ]
// MicroBNVInt::BNV_Chi::Rate_Eps MicroBNVInt::BNV_Chi::GetRate_Eps(
// 	const double &m_chi,
// 	const Baryon &B
...
// }
//--------------------------------------------------------------
/// @brief Integrate the B → χ decay rate over the star and return
///        (i) the total baryon-number-loss rate per baryon [yr⁻¹]
///        (ii) the corresponding limit on ε for this channel.
///
/// This does:
/// 1. grab r, M(r), ν(r) from the pulsar’s StarProfile
/// 2. get the *local* decay rate vs radius from Rate_vs_R(m_chi, B)
/// 3. apply GR volume element 4π r² (1 - 2M/r)^(-1/2) and redshift e^{ν}
/// 4. convert km³·fm⁻³ → dimensionless via 1e54
/// 5. integrate over r
/// 6. convert s⁻¹ → yr⁻¹
/// 7. divide by total baryon number B_tot from the sequence point
/// 8. rescale ε:  ε_limit = ε₀ √( Γ_sd / Γ_model )
MicroBNVInt::BNV_Chi::Rate_Eps MicroBNVInt::BNV_Chi::GetRate_Eps(
	const double &m_chi,
	const Baryon &B)
{
	...
	if (!r_col || !m_col || !nu_col)
	{
		Z_LOG_ERROR("MicroBNVInt::BNV_Chi::GetRate_Eps: missing core columns (r/M/nu) in profile.");
		return {0.0, std::numeric_limits<double>::max()};
	}

	// make working copies to do arithmetic
	Zaki::Vector::DataColumn r = *r_col;	 // [km]
	Zaki::Vector::DataColumn M_r = *m_col;	 // [M_sun]
	Zaki::Vector::DataColumn nu_r = *nu_col; // dimensionless

	// convert M(r) → km-mass so that (1 - 2M/r) makes sense
	M_r *= Zaki::Physics::SUN_M_KM; // now M_r is in [km]

	// 2) local decay rate as a function of r
	// Rate_vs_R(...) is assumed to return {r, Γ(r)} with Γ(r) in [1/fm³ · s⁻¹] or similar
	Zaki::Vector::DataSet rate_ds = Rate_vs_R(m_chi, B);
	...
	// pack in a dataset for interpolation/integration
	Zaki::Vector::DataSet decay_integral_ds({r, decay_integrand_dc});
	decay_integral_ds.Interpolate(0, 1);
	...
	const double r_min = r[0];
	const double r_max = r[-1];
	double result_sinv = decay_integral_ds.Integrate(1, {r_min, r_max}); // [s⁻¹]
	...
	// 8) turn that into an ε limit
	// Γ_model(ε) ∝ ε² → ε_lim = ε₀ √(Γ_sd / Γ_model)
	double output_eps = 0.0;
	if (output_rate <= 0.0)
	{
		output_eps = std::numeric_limits<double>::max();
	}
	...
	{
		output_eps = default_eps * std::sqrt(gamma_bnv_bin_lim / output_rate);
	}
...
//--------------------------------------------------------------
// Saves the results
void MicroBNVInt::BNV_Chi::Export(const Zaki::String::Directory &in_dir) const
{
