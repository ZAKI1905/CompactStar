...
namespace
{
std::size_t Find(const Analysis::NumberMetadata& m, const std::string& name)
{
...
std::string SerializeDomain(const Analysis::NumberDomain& d)
{
    std::ostringstream s;
    s.precision(17);
...
std::string SequenceIdentity(const Analysis::NumberMetadata& m)
{
    std::ostringstream s;
    s.precision(17);
    ...
    double B0_count, std::string star_identity, std::string source_domain_identity)
{
    ...
    if (m.domain.type!=Analysis::DomainType::WholeStar) throw std::runtime_error("tangent requires whole-star domain");
...
std::string EquilibriumBaryonTangent::SerializeDomainIdentity(const Analysis::NumberDomain& d)
