...
Historical status clauses in the index retain their dated scope. Phase-4E closeout is ready;
Phase 5 is not begun.
...
| Ordinary-star TOV surface event / termination contract — **ADR-0009 ACCEPTED, SOURCE CONFORMED, NUMERICALLY VALIDATED, ARTIFACT MIGRATION COMPLETE** | 4D-RV + 4D-DA independently VERIFIED; 4D-BL first baseline established. Phase-4E closeout ready; `docs/validation/PHASE4D_MONOPOLE_BASELINE.md:272` |
| ~~ADR-0008 Validation D monotonicity clause vs the measured O(h) node-placement floor~~ — **clarified by owner adjudication, Phase 4D-DA 2026-09-04** (`docs/validation/PHASE4D_CORRECTED_MONOPOLE_REVALIDATION.md` §21; EOS-knot partition stays optional) | first baseline established (4D-BL); smallest Phase-4E closeout ready, `docs/validation/PHASE4D_MONOPOLE_BASELINE.md:272` |
