...
| [ADR-0009](ADR-0009-tov-surface-event-and-termination.md) | Ordinary-star TOV surface event and integration-termination contract | **PROPOSED** | 2026-09-03 — from the TOV-RR-01 audit: `TOVSolver::ODE` makes a *trial* `p < p_cut` a fatal `GSL_EBADFUNC`, so the accepted-state cutoff test never fires, `R_*` is one output step short and partition-dependent (`1.6e-3`), and stars truncate at the crust–core transition for scattered `(radial_res, ε_c)` pairs incl. ≈3 % of central densities at the default 10000. Proposes: `R_*` = the crossing `p(R_*) = p_cut` of the accepted solution (Q1), `p_cut` unchanged (Q2), no fatal guard (Q3), trial-only `ε(max(p, p_floor))` continuation (Q4), a deterministic bracketed event locator — pressure-coordinate landing, measured partition-invariant to `8e-11` (Q5) — the partition as sampling only (Q6), no driver-reset contract (Q7), fail-closed publication with an explicit completion status (Q8–Q9), a stored surface point at `p = p_cut` (Q10), one governed regeneration of every artifact carrying `R_*` after a predeclared sweep (Q11–Q12), INV-06's locator amended with its value kept (Q13), the corrected Phase-4D revalidation and first monopole baseline sequenced behind it (Q14). Evidence: `docs/validation/TOV_SURFACE_CONTRACT_DERIVATION.md`. **Owner adjudication of Q1–Q14 required** |
...
| Ordinary-star TOV surface event / termination contract (INV-06 locator, fatal RHS guard, partition semantics, fail-closed publication) — **ADR-0009 PROPOSED 2026-09-03** | owner adjudication |
