...
| Ordinary-star TOV surface event / termination contract — **ADR-0009 ACCEPTED, SOURCE CONFORMED, NUMERICALLY VALIDATED, ARTIFACT MIGRATION COMPLETE** | corrected Phase-4D independent revalidation executed (4D-RV: CHARACTERIZED); first monopole baseline gated on the owner's adjudication of ADR-0008 Validation D; `docs/validation/PHASE4D_CORRECTED_MONOPOLE_REVALIDATION.md` |
| ADR-0008 Validation D monotonicity clause vs the measured O(h) node-placement floor of the profile-partition measure weight and the sampled first-order background — post-validation clarification (or the optional EOS-knot partition, Q2b, as a governed increment) | first monopole baseline; Phase 4E; `docs/validation/PHASE4D_CORRECTED_MONOPOLE_REVALIDATION.md` §20 |
