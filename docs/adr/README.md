...
| [ADR-0007](ADR-0007-hartle-second-order-monopole-response.md) | Hartle O(Ω²) monopole structural response at fixed central energy density | **PROPOSED** | — drafted 2026-09-02 from the primary source (Hartle 1967); **owner adjudication required** (Q1–Q13); invokes `GOVERNANCE.md` §3.1 once accepted; evidence `docs/validation/PHASE4C_HARTLE2_DERIVATION.md` |
...
| ~~Hartle O(Ω²) — perturbation variable, corrected l = 0 equations, fixed-central-density boundary condition, exterior `δM` term, EOS `dε/dp` authority, `Ω²`-normalized exposure (INV-08, INV-09)~~ — **ADR-0007 PROPOSED 2026-09-02** (`docs/validation/PHASE4C_HARTLE2_DERIVATION.md`); surfaced follow-ups: the EOS-derivative API (4C-I-0 prerequisite) and a separate later `l = 2` (shape / quadrupole) ADR if the owner reads the Phase-4 exit criterion that way | owner adjudication |
