...
> BA12 and BA12R remain permanently **FAIL**.  The Phase-6 main integration is
> **UNINTERRUPTED RKF45**; observation is **PASSIVE**; exact accepted endpoints are used
> directly; and strict-interior reconstruction is **SELF-QUALIFIED TWO-LEVEL ISOLATED rk8pd**.
> The bounded qualification exactly reproduced Arm E, all 239 strict-interior checkpoints, all
> three Cstar-knot observations, all 478 O1/O2 state values, all 241 diagnostic packets, and R20
> (`validation/PHASE6_ADR0017_PRODUCTION_IMPLEMENTATION.md`,
> `validation/PHASE6_ADR0017_PRODUCTION_ACCEPTANCE.md`).  Canonical Phase-6 numerical BNV
> candidate: **NONE**.  Governed BNV numerical baseline: **NONE**.  The EKU cluster is **NOT YET
> QUALIFIED**.  The clean BA12R campaign and its six fresh uninterrupted
> BASELINE/REFINED/ULTRA source/control trajectories are **NOT YET AUTHORIZED**.  No cluster job
> is authorized here.  Phase-5D, `ScaledRKF45`, Cstar, R20 quadrature, and ADR-0015/0016
...
> candidate-only. The accepted exclusions include `n -> chi + gamma`, realistic A18,
> superfluidity, Regime-II/MixedStar, variable-`Z`/sliding-background evolution, and
> depletion beyond `|DeltaB|/B0 <= 1e-6`. Evidence: the
...
> canonical unreduced source-basis `G_y`, global-after-integration baryon reduction, canonical
> chemical `Z`, the structurally provenance-complete `W` adapter, active branches, coefficient
> redshift/sign semantics, lifetime/refusal requirements, and the revised GC validation ladder.
> Production `G_y/Z/W` is **NOT IMPLEMENTED**. Realistic Track-R chemical closure remains
> **BLOCKED ON AUTHENTICATED A18 + delta-v + UIX* AUTHORITY**; free gas is an intermediate
> correction-separation fixture and cannot substitute. INV-09 remains **VERIFIED / RESOLVED**;
> INV-11 remains **UNRESOLVED** because secular-evolution semantics are still open. BNV is
> **NOT BEGUN**. Evidence: `docs/adr/ADR-0013-corrected-rotochemical-chemical-coefficients.md`;
...
> **Phase 4D-DA (2026-09-04):** ADR-0008 Validation D's monotonicity clause adjudicated (a heuristic
> the accepted profile-partition measure cannot mathematically satisfy; clarified into D′1–D′5, all met by
> the 4D-RV evidence): status `HARTLE O(OMEGA^2) MONOPOLE RESPONSE VERIFIED` (`docs/validation/PHASE4D_CORRECTED_MONOPOLE_REVALIDATION.md` §21). **The one next action is the first monopole baseline,
> in a separate task. Phase 5 remains blocked behind 4E.**
>
> **Phase 4D-RV (2026-09-03):** the corrected independent Hartle-monopole revalidation was
> executed on the migrated ADR-0009 backgrounds: `HARTLE O(OMEGA^2) MONOPOLE RESPONSE CHARACTERIZED — INDEPENDENT VALIDATION INCOMPLETE`
> (`docs/validation/PHASE4D_CORRECTED_MONOPOLE_REVALIDATION.md`). All physics lines pass; ADR-0008 Validation D's monotonicity clause is NOT MET at the
> measured node-placement floor. **No monopole baseline; 4E stays blocked; the one next action is
> the owner's adjudication of D. Phase 5 remains blocked.**
>
> **TOV-SURF-MR (2026-09-03):** ADR-0009 artifact migration is complete following
> the owner-adjudicated envelope. Seven current hashes, exact producer reproduction,
> main 41/41 and self-contained 20/20 passes: `docs/validation/TOV_SURFACE_ARTIFACT_MIGRATION.md:889`,
> `docs/validation/TOV_SURFACE_ARTIFACT_MIGRATION.md:960`. Corrected Phase-4D independent monopole revalidation is ready;
> it must pass before the first monopole baseline. Phase 5 remains blocked.
> The stop/adjudication history is preserved: `docs/validation/TOV_SURFACE_ARTIFACT_MIGRATION.md:1003`.
...
- ◐ **Grid-convergence harness — PARTIAL (2B-4A).** The **nonrotating TOV → cooling slice is
  ...
  **The scope of this item is therefore left UNCHANGED pending owner adjudication** — relocating
  ...
  Per INV-13, interpolation remains linear and sampled integrals retain finite-grid
  error. **Current nonrotating convergence authority (TOV-SURF-MR):** both fixed-εc
  and target-mass arms contain complete governed-event stars at
  1250/2500/5000/10000/20000/40000/80000. The historical truncated 2500 rows were
  replaced; 2500 is now on the same physical branch without missing-crust
  compensation. The historical coarse-grid radius-error detector is retired.
  Installed M/R partition spreads are within ADR-0009's unchanged bounds.
  Finest-grid diagnostics: B 2.23462, Cstar 2.24613, Lnu 0.16443,
  dlnT/dt 0.14786, terminal Tinf 0.96465 and trajectory norm 0.14624;
  M/R/Lgamma have no reliably measurable common order. These are measured
  finite-resolution behaviors, not a universal asymptotic order or a continuum
  accuracy budget. Regression tolerances remain independently tied to repeatability.
  Current 14-row debug and 63-row trajectory artifacts, producer and evidence:
  `docs/validation/TOV_SURFACE_ARTIFACT_MIGRATION.md:907`. Original Phase-2B measurements and their later defect diagnosis
  remain in `docs/validation/GRID_CONVERGENCE.md` and
  `docs/validation/TOV_RADIAL_RES_2500_AUDIT.md` as historical records.
...
## Phase 4 — Rotation correctness — COMPLETE (accepted ordinary-NStar scope)
...
**Current status:** all increments 4A–4E COMPLETE; `docs/validation/PHASE4_CLOSEOUT.md:33`.

**Historical entry status: ENTRY AUDIT COMPLETE (increment 4A-0, 2026-09-02) —
...
**Exit criteria SATISFIED for accepted scope** (`docs/validation/PHASE4_CLOSEOUT.md:33`): O(Ω) and l=0 O(Ω²) validated and reachable. The candidate status of `675b4a9` is
...
- **Historical Phase-5B-RAT: STRUCTURAL RESPONSE HUMAN-RATIFIED — GOVERNED INTEGRATION NEXT.** Independent
  ...
  the reviewed ordinary-`NStar` / Track-R scope. A separate governed task must install the first
  structural regression artifact, rerun both complete suites, authenticate all baselines, and
  integrate canonically before INV-09 may close. Until then INV-09 remains **INTENDED BUT
...
- **PHASE-5D CONTROLLED NON-SUPERFLUID EVOLUTION GOVERNED / COMPLETE VALIDATION PASS /
  CANONICAL INTEGRATION-READY / CLOSED FOR DECLARED CONTROLLED FROZEN-V1 SCOPE.**
  ...
  nonblocking caveats and the owner ratified the controlled scope. Independent fresh contexts
  produced byte-identical governed artifacts; the expensive regression reproduces the eleventh
  governed baseline from an empty scratch root. Complete validation passes 51/51 data-free and
  75/75 authenticated tests. Global INV-11 remains unresolved for broader extensions.
...
- **Controlled v1 state, F/H rates and coupled chemical/thermal RHS are governed and completely validated**
...
- **Independent review, owner ratification, reproducibility repair, baseline promotion, and
  complete validation are finished for controlled frozen-v1 scope.** Canonical fast-forward is
  the sole remaining integration action for this branch.
...
**Phase-6A-1 status (2026-09-25):** **ADR-0015 ACCEPTED / ADR-0016 ACCEPTED / ADR-0017
ACCEPTED / HUMAN-RATIFIED; ADR-0017 PRODUCTION IMPLEMENTATION QUALIFIED / OWNER-ACCEPTED /
CANONICAL INTEGRATION AUTHORIZED.** Historical BA12 and BA12R remain permanently **FAIL**.  The
Phase-6 main integration is **UNINTERRUPTED RKF45**, observation scheduling is **PASSIVE**, exact
accepted endpoints are used directly, and strict-interior reconstruction is **SELF-QUALIFIED
TWO-LEVEL ISOLATED rk8pd**.  Diagnostics are evaluated from qualified checkpoint states and R20
retains its existing observation grid and composite-trapezoid semantics.  The bounded
qualification and owner acceptance are recorded in
`docs/validation/PHASE6_ADR0017_PRODUCTION_IMPLEMENTATION.md` and
`docs/validation/PHASE6_ADR0017_PRODUCTION_ACCEPTANCE.md`.  Canonical Phase-6 numerical BNV
candidate: **NONE**.  Governed BNV numerical baseline: **NONE**.  The EKU cluster is **NOT YET
QUALIFIED**.  A clean BA12R campaign and six fresh uninterrupted BASELINE/REFINED/ULTRA
source/control trajectories are **NOT YET AUTHORIZED**.  Phase-5D, `ScaledRKF45`, Cstar, R20
quadrature, ADR-0015/0016 scientific meaning, and BNV physics are unchanged.  After canonical
integration, the next authorized task is documentation/planning for EKU-cluster numerical-
platform qualification; no job submission follows automatically.
...
sign/index, thermal-ledger, and frozen-v1 policy. The controlled implementation is now governed,
completely validated, independently reviewed, and human-ratified for frozen-v1; its scoped
subparts resolve on canonical fast-forward, while the broader realistic extensions remain open.
**INV-07 is no longer a gate** — resolved by
