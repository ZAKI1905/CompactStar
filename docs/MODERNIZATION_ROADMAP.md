...
| **4E** | **◄ NEXT — split.** (a) **ADR-0007 amendment + 4D-J closeout** (scientific-semantic, `GOVERNANCE.md` §3.1 continues): adjudicate internal density discontinuities of a tabulated EOS (Hartle's `dE/dP` delta functions — internal shells `4πr_i²Δε_i ξ₀(r_i)`, or term 1 integrated against `dε` on the EOS table), implement, re-run 4D Experiment J and the sourced Stieltjes check, **then the first monopole baseline** (§3.1 condition 7). (b) The Phase-5 structural fields themselves — `m₀/Ω²`, `δp₀/Ω²`, `ξ₀/Ω²`, `δM/Ω²`, `ω̄/Ω`, `I` — are **already carried by `HartleMonopoleResponse` / `PhysicalHartleMonopole` (4C-I1) and verified at the implementation level (4D)**; 4E reduces to the closeout plus a one-line ratification that those fields are the Phase-5 interface. **Phase 5 must not begin before (a).** | 4D ✅ (characterized) |
...
0.5 governance ─► 1 build ─► 2A pre-baseline ─► 2B baseline ─► 3 consolidation ─► 4 rotation ─► 5 rotochemical ─► 6 BNV
  ✅ RATIFIED    ✅ COMPLETE      ✅ COMPLETE      ◐ ACTIVE (gate met)   ✅ COMPLETE (merged df859b5)   ◐ 4A–4D DONE — 4D FAILED ON STEPPED CRUSTS — ADR-0007 AMENDMENT NEXT
                                    │                                 │                │              │
