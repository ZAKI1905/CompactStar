...
- ◐ **Grid-convergence harness — PARTIAL (2B-4A).** The **nonrotating TOV → cooling slice is
  ...
  **zero** differential error above 5000 (bit-identical `ε_c`). Default-grid continuum error:
  ...
  `R_*` and needs an ADR. **That ADR is drafted: `docs/adr/ADR-0009-tov-surface-event-and-termination.md` (PROPOSED, TOV-SURF-G 2026-09-03; evidence `docs/validation/TOV_SURFACE_CONTRACT_DERIVATION.md`).** Once adjudicated and implemented, this artifact is regenerated on the corrected solver with `2500` retained as a valid coarse point and the refinement set extended (evidence §13). CTest `grid_convergence_cmf` (labels
