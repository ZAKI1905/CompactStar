...
> **TOV-SURF-I status (2026-09-03):** ADR-0009 Q1–Q14 accepted; implementation
> validation stopped on the 2.0-M☉ V7 mass-impact discrepancy. Candidate source
> restored, all seven artifacts unchanged. Owner review is the next gate; source
> conformance and full validation are still required before artifact migration,
> corrected Phase-4D revalidation, and the first monopole baseline. Phase 5 remains
> blocked. Evidence: `docs/validation/TOV_SURFACE_IMPLEMENTATION.md:88`, `:227`.
...
- ◐ **Grid-convergence harness — PARTIAL (2B-4A).** The **nonrotating TOV → cooling slice is
  ...
  **zero** differential error above 5000 (bit-identical `ε_c`). Default-grid continuum error:
  ...
  `R_*` and needs an ADR. **That ADR is accepted: `docs/adr/ADR-0009-tov-surface-event-and-termination.md` (ACCEPTED, TOV-SURF-I 2026-09-03; implementation stopped at V7; evidence `docs/validation/TOV_SURFACE_CONTRACT_DERIVATION.md`).** Once implemented and validated, this artifact is regenerated on the corrected solver with `2500` retained as a valid coarse point and the refinement set extended (evidence §13). CTest `grid_convergence_cmf` (labels
