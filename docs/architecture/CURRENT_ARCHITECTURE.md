...
> BNV candidate: **NONE**. Governed BNV baseline: **NONE**. No production checkpoint
> reconstruction has begun. A future clean BA12R requires fresh uninterrupted
...
| ADR-0015 BNV open-system thermal layer | **GOVERNED CONTRACT / NOT IMPLEMENTED** | The accepted contract owns the local open-system first law, direct-energy partition semantics, moving-reference `t`, the baryon-neutral remainder `S_y-t Bdot=L sigma`, the physical seam `{Bdot,sigma}`, diagnostic-only `k`, and matched-control/sign requirements for controlled abstract-source Regime I. No production BNV source, rate, trajectory, `n->chi-gamma`, Regime-II/MixedStar thermal evolution, or Phase-6 baseline exists. `docs/adr/ADR-0015-bnv-open-system-thermal-ledger.md` |
| Phase-6A-1 controlled BNV implementation plan | **OWNER-ACCEPTED / IMPLEMENTATION AUTHORIZED / NOT YET BEGUN** | The bounded plan governs the first abstract Regime-I neutron-sink implementation, including P0/P1/P2 fixtures, matched no-BNV controls, the frozen-background budget, BA1-BA17, and M1-M21. It selects no physical BNV rate/model and creates no numerical baseline. The first result remains candidate-only. `docs/validation/PHASE6A1_CONTROLLED_BNV_IMPLEMENTATION_ACCEPTANCE.md` |
| Phase-6 checkpoint-output layer | **ADR-0017 ACCEPTED / HUMAN-RATIFIED; NOT YET PRODUCTION-IMPLEMENTED** | Main integration: **UNINTERRUPTED RKF45 / RATIFIED, NOT YET IMPLEMENTED IN PRODUCTION**. Observation scheduling: **PASSIVE / RATIFIED**. Exact accepted endpoints are returned directly. Strict-interior reconstruction: **TWO-LEVEL ISOLATED rk8pd / RATIFIED, NOT YET PRODUCTION-IMPLEMENTED**. The existing rk8pd record is self-qualified numerical evidence supporting the ADR, not a qualified production implementation. Phase-5D, `ScaledRKF45`, Cstar, R20 quadrature, and BNV physics remain unchanged. A future clean BA12R requires fresh uninterrupted BASELINE/REFINED/ULTRA source and matched control trajectories after separate production qualification. `docs/adr/ADR-0017-phase6-passive-checkpoint-reconstruction.md:1-12` |
