...
> BA12R remain **FAIL**. Passive scheduling is **VALIDATED**: requested observation times are
> passive metadata and the qualified observer preserved the final state, accepted/rejected
> counts, and accepted-step history exactly
> (`docs/validation/PHASE6A1_PASSIVE_OBSERVATION_EXPERIMENT.md:275-318`). The original checkpoint
> comparison remains **NO CHECKPOINT RECONSTRUCTION METHOD QUALIFIED** and selected no method
> (`docs/validation/PHASE6A1_CHECKPOINT_RECONSTRUCTION_CANDIDATE_RECOVERY.md:335-356`). Two-level
> local rk8pd is **SELF-QUALIFIED NUMERICAL EVIDENCE SUPPORTING ADR-0017**. The ratified Phase-6
> main architecture is **UNINTERRUPTED RKF45 / BOUNDED PRODUCTION QUALIFICATION PASS /
> PRODUCTION IMPLEMENTATION CANDIDATE AWAITING OWNER ACCEPTANCE**;
> observation scheduling is **PASSIVE / RATIFIED**; exact accepted endpoints are used directly;
> and strict-interior reconstruction is **TWO-LEVEL ISOLATED rk8pd / BOUNDED PRODUCTION
> QUALIFICATION PASS / PRODUCTION IMPLEMENTATION CANDIDATE AWAITING OWNER ACCEPTANCE**
...
> BNV candidate: **NONE**. Governed BNV baseline: **NONE**. The Phase-6-owned candidate
> implementation is `CompactStar/Physics/BNV/PassiveCheckpointOutput.*`; it retains immutable
> accepted-step records, brackets passive observations, returns exact main endpoints, creates
> fresh local rk8pd contexts and GSL objects for O1/O2 strict-interior solves, fails closed on
> self-qualification, evaluates diagnostics from the selected checkpoint state, and preserves
> checkpoint-grid composite-trapezoid R20. The bounded Mac qualification reproduced Arm E and
> all authenticated reconstruction/diagnostic/R20 oracle values exactly
> (`docs/validation/PHASE6_ADR0017_PRODUCTION_IMPLEMENTATION.md`). It is not governed or canonical
> until explicit owner acceptance and canonical integration. A future clean BA12R requires fresh uninterrupted
> BASELINE/REFINED/ULTRA source and matched control trajectories after separate production
> qualification; those runs are not authorized here. Phase-5D source/baseline/producer/comparator,
> `ScaledRKF45`, Cstar, R20 quadrature, and ADR-0015/0016 meaning are unchanged.
...
> HUMAN-RATIFIED**. This is architecture/physics-contract authority only: BNV production
> implementation is **NOT BEGUN**; no physical rate is selected; `n->chi-gamma`, realistic A18,
> Regime-II transport, and MixedStar thermal evolution are **NOT IMPLEMENTED / NOT BEGUN**.
> Existing compiled legacy BNV channels remain unexercised and do not implement ADR-0015.
> Evidence: [ADR-0015](../adr/ADR-0015-bnv-open-system-thermal-ledger.md) and the
> [ratification record](../validation/PHASE6A0_BNV_THERMAL_FIRST_LAW_RATIFICATION.md).

> **Phase-5D-1INT branch status (2026-09-12): CONTROLLED NON-SUPERFLUID ROTOCHEMICAL EVOLUTION GOVERNED / COMPLETE VALIDATION PASS / CANONICAL INTEGRATION-READY / CLOSED FOR DECLARED CONTROLLED FROZEN-V1 SCOPE.** The fresh-producer regression protects the eleventh governed baseline at SHA-256 `2606916915b2da5c051b1a06637c0a371a74751c6e77b2775bc629a63bc9f6dd`. Complete validation passed 51/51 data-free and 75/75 authenticated tests, followed without cleanup by another exact regeneration. Python CTest launch isolation prevents source-tree bytecode contamination without modifying protected scripts. The scoped INV-11b-f controlled/frozen-v1 subparts become resolved upon canonical fast-forward; global INV-11 remains **UNRESOLVED**. The `P0=1 ms` trajectory remains a super-Kepler mathematical control with `physical_spin_interpretation=false`; provenance is FR2005 Eq. (49) / PCY97 fully accreted with zero numerical change; scaled RKF45 remains accepted only for this benchmark. Realistic FR2005/A18 remains **SOURCE-LIMITED / BLOCKED**; BNV **NOT BEGUN**. See [integration record](../validation/PHASE5D1_CONTROLLED_ROTOCHEMICAL_EVOLUTION_INTEGRATION.md), [preparation record](../validation/PHASE5D1_GOVERNED_ARTIFACT_PREPARATION.md), and [ratification](../validation/PHASE5D1_CONTROLLED_ROTOCHEMICAL_EVOLUTION_RATIFICATION.md). Earlier branch/canonical-entry statements below are historical and retain their original scope.
...
> independently reviewed, and human-ratified for governed generic/free-gas scope. GC1-GC12 and
> GC14 pass within declared classifications; GC13 remains **SOURCE-LIMITED / BLOCKED** on A18.
> The deterministic artifact is not installed as a governed baseline. INV-09 remains **VERIFIED /
> RESOLVED**, INV-11 remains **UNRESOLVED**, and no eta evolution, weak rates, heating/cooling,
> realistic A18 closure, or BNV is authorized. Evidence:
...
> validation passed 9/9 focused, 39/39 data-free, and 62/62 complete. INV-09 is **VERIFIED /
> RESOLVED**; INV-11 remains **UNRESOLVED**. Review qualifications remain controlling: PB9 is
> tautological; mutation totals mix production-discriminating and local algebraic/null checks;
> PB9/PB10 budgets are conservative, with PB10 independent `K` reconstruction, PB11 nonlinear
> closure, and achieved charge residual the strongest evidence; `B_B conditioning = 1` is a
> cancellation ratio; PB8 can be floor-limited; generic declared physical jumps require
> authenticated adapter metadata while ordinary Track-R onsets are continuous; PB13 is a
> conservative enclosure; EOS/tail authority is adapter-owned; and PB6 is partition/refinement
> validation, not an independent physics oracle. No source-qualified core `I_Omega` or `M_max` is
> claimed. Corrected R2006 susceptibility / chemical response is the next layer but
> is not begun. Evidence: `docs/validation/PHASE5B_INV09_GLOBAL_RESPONSE_INTEGRATION.md`.
...
| Phase-5D controlled pipeline (`FrozenRotochemicalRunContext`, `SecularEvolutionDriver`, `ScaledRKF45`) | **LIVE — GOVERNED; COMPLETE VALIDATION PASS; CANONICAL INTEGRATION-READY; CLOSED FOR DECLARED CONTROLLED FROZEN-V1 SCOPE** | The Phase-5D-owned immutable context retains semantic G/Z/W/fixed-response and same-`Ltilde` authorities, thermal-source bytes, one prescribed-spin owner, and the typed `(ln(Tinf/1e8 K),eta_npe,eta_npmu)` layout. The controlled driver evolves the two-channel modified-Urca ledger on the fixed free-gas background. Independent empty-scratch contexts regenerated byte-identical scientific artifacts; the governed regression protects baseline SHA-256 `2606916915b2da5c051b1a06637c0a371a74751c6e77b2775bc629a63bc9f6dd`; and complete suites passed 51/51 data-free and 75/75 authenticated. `docs/validation/PHASE5D1_CONTROLLED_ROTOCHEMICAL_EVOLUTION_INTEGRATION.md` |
| ADR-0015 BNV open-system thermal layer | **GOVERNED CONTRACT / PHASE-6 CONTROLLED IMPLEMENTATION RECOVERED AS CANDIDATE** | `Physics/BNV` owns the abstract controlled source, moving reference, direct-energy ledger, frozen validity, controlled RHS, and diagnostic packet. It remains a controlled Regime-I candidate: no physical BNV rate/model, `n->chi-gamma`, Regime-II/MixedStar thermal evolution, or Phase-6 baseline exists. `docs/adr/ADR-0015-bnv-open-system-thermal-ledger.md` |
| Phase-6A-1 controlled BNV implementation | **PRODUCTION SOURCE CANDIDATE / NOT GOVERNED** | The recovered Phase-6-owned implementation is compiled from source evidence on the ADR-0017 qualification branch; no noncanonical ancestry is promoted. It implements the accepted abstract P0/P1/P2 fixtures and diagnostics but selects no physical BNV rate/model and creates no numerical baseline. `docs/validation/PHASE6A1_CONTROLLED_BNV_RECOVERY_IMPLEMENTATION.md` |
| Phase-6 checkpoint-output layer | **ADR-0017 ACCEPTED / BOUNDED PRODUCTION QUALIFICATION PASS / PRODUCTION IMPLEMENTATION CANDIDATE AWAITING OWNER ACCEPTANCE** | `PassiveCheckpointOutput` owns one uninterrupted persistent RKF45 main state with one terminal ceiling, immutable accepted-step history, a passive bracket-only schedule, exact-endpoint bypass, fresh isolated two-level rk8pd strict-interior reconstruction, fail-closed self-qualification, reconstructed-state diagnostic evaluation, explicit provenance/uncertainty, and unchanged checkpoint-grid composite-trapezoid R20. The bounded Mac run exactly reproduced Arm E, all 239 O1/O2 state and diagnostic pairs, the three one-knot cases, and R20; Phase-5D, `ScaledRKF45`, Cstar, R20 semantics, and BNV physics are unchanged. Owner acceptance and canonical integration remain separate. `docs/validation/PHASE6_ADR0017_PRODUCTION_IMPLEMENTATION.md` |
...
- The controlled non-superfluid rotochemical pipeline is **IMPLEMENTED / GOVERNED / COMPLETELY
  VALIDATED / INDEPENDENTLY REVIEWED / HUMAN-RATIFIED / CANONICAL INTEGRATION-READY / CLOSED FOR
  DECLARED CONTROLLED FROZEN-V1 SCOPE** on this branch.
  ...
  authorities. The controlled artifact is governed-baseline promoted and ready for the authorized
  canonical fast-forward; global INV-11 remains unresolved for broader extensions.
