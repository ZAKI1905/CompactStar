...
> **Track-R Structure-1 (2026-09-05): static free-gas Table-1 row reproduced under the declared common-central-state publication-bin test.** The new value-only barotrope feeds a generated table into the unchanged canonical TOV primitive; independent phase-space/enthalpy tests validate the static structure and the zero-pressure tail. This structure result remains a scientific candidate for human review. INV-09/INV-11 and global response/evolution scope are unchanged. Evidence: `docs/validation/TRACKR_FREEGAS_WHOLESTAR_STRUCTURE1.md:1`, sections 3–22.
...
| `TrackRFreeGasThermodynamicProvider` | **LIVE — HUMAN-RATIFIED WHOLE-STAR LOCAL COVERAGE** | Cold ideal vacuum/p-e/npe/npe-mu source model; separate 1D/2D/3D active charts, value-only thresholds, explicit near-onset numerical failures, and the pre-Sigma-minus endpoint. The additive `BarotropeAt` capability now supplies equilibrium values independently of chemical H. The separate Structure-1 generated-table/canonical-TOV path passes the static Table-1 comparison, without global response or evolution; the existing response guards remain unchanged. Structure-1 is a candidate for human review. See `docs/validation/TRACKR_FREEGAS_WHOLESTAR_STRUCTURE1.md:1`, sections 3–10 and 22. `CompactStar/EOS/TrackRFreeGasThermodynamics.hpp`; `docs/validation/PHASE5A5_TRACKR_LOCAL_RATIFICATION.md:27-46`, `:94-135` |
