...
> | O(Ω²) monopole system — primary-source derivation, surface audit, governed replacement (ADR-0007 PROPOSED) | **`bb073c8`** | Phase 4C-G — `docs/validation/PHASE4C_HARTLE2_DERIVATION.md` |
...
- It does **not** claim second-order Hartle is validated. It is publicly callable, has zero
  ...
  Phase 4C-G derived the governed replacement from the primary source and **ADR-0007 was
  ACCEPTED on 2026-09-02** — a contract, not a result. Until it is implemented (4C-I1, preceded
  by the 4C-I0 EOS-derivative authority) and independently validated (4D), **no O(Ω²) number
  from this repository is physics**, and the `l = 2` sector is out of scope rather than verified.
