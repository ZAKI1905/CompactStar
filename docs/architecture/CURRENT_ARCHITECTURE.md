...
- It does **not** claim second-order Hartle is validated. It is publicly callable, has zero
  ...
  implementation**. That is a source that now matches an accepted contract — still not a
  verified result. Until Phase 4D supplies independent validation, **no O(Ω²) number from this
  repository is physics**, none may be cited, **no monopole baseline exists**, and the `l = 2`
  sector is out of scope rather than verified.
