...
> **Phase-6A-1I branch status (2026-09-19): BOUNDED CONTROLLED BNV IMPLEMENTATION
> PRESENT / BA12 FAILED / MATERIAL ARCHITECTURE CONFLICT / NO CANDIDATE.** The
> branch implements the accepted abstract Regime-I source, moving-reference,
> actual-potential ledger, zero-spin wrapper, frozen validity and trajectory seams.
> BA1-BA10b passed and M1-M21 were detected before trajectories. The predeclared P2
> run then failed BA12 with component-scaled `x_state` difference
> `1.7255917120989046 > 1`; validation stopped without a candidate artifact or
> baseline. Final BA15 also proved that the accepted new `Analysis` tangent paths
> necessarily change Phase-5D's recursively collected scientific-source provenance;
> the governed producer/comparator cannot be changed under this authority. No physical
> BNV rate/model was selected. See the
...
| ADR-0015 BNV open-system thermal layer | **GOVERNED CONTRACT / BOUNDED BRANCH IMPLEMENTATION PRESENT / NOT CANONICAL** | The branch implements the accepted local open-system first law, direct-energy partition semantics, moving-reference `t`, baryon-neutral remainder and matched-control seam for the controlled abstract Regime-I experiment. It contains no physical BNV rate, `n->chi-gamma`, Regime-II/MixedStar thermal evolution or Phase-6 baseline. Numerical candidate acceptance failed BA12. `docs/adr/ADR-0015-bnv-open-system-thermal-ledger.md` |
| Phase-6A-1 controlled BNV implementation | **PRETRAJECTORY PASS / BA12 FAIL / BA15 ARCHITECTURE CONFLICT / NO CANDIDATE** | The exact P2 ODE-refinement norm was `1.7255917120989046 > 1`. Final regression also showed that the accepted new `Analysis` tangent paths necessarily alter governed Phase-5D source provenance. No candidate artifact, owner ratification, canonical integration or baseline follows. `docs/validation/PHASE6A1_CONTROLLED_BNV_NUMERICAL_FAILURE.md` |
