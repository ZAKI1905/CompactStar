...
> main architecture is **UNINTERRUPTED RKF45 / PRODUCTION IMPLEMENTATION CANDIDATE**;
> observation scheduling is **PASSIVE / RATIFIED**; exact accepted endpoints are used directly;
> and strict-interior reconstruction is **TWO-LEVEL ISOLATED rk8pd / PRODUCTION
...
> checkpoint-grid composite-trapezoid R20. It is not governed or canonical until bounded
> qualification and owner acceptance. A future clean BA12R requires fresh uninterrupted
...
| Phase-6 checkpoint-output layer | **ADR-0017 ACCEPTED / PRODUCTION IMPLEMENTATION CANDIDATE** | `PassiveCheckpointOutput` owns one uninterrupted persistent RKF45 main state with one terminal ceiling, immutable accepted-step history, a passive bracket-only schedule, exact-endpoint bypass, fresh isolated two-level rk8pd strict-interior reconstruction, fail-closed self-qualification, reconstructed-state diagnostic evaluation, explicit provenance/uncertainty, and unchanged checkpoint-grid composite-trapezoid R20. Focused and bounded numerical qualification remain mandatory before owner acceptance; Phase-5D, `ScaledRKF45`, Cstar, R20 semantics, and BNV physics are unchanged. `docs/adr/ADR-0017-phase6-passive-checkpoint-reconstruction.md:1-12` |
