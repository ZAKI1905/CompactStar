...
- It does **not** claim second-order Hartle is validated. It is publicly callable, has zero
  ...
  Phase 4C-G derived the governed replacement from the primary source and proposed ADR-0007;
  until that ADR is accepted, implemented (4C-I) and independently validated (4D), no O(Ω²)
  number from this repository is physics.
