...
> reconstruction has begun. Phase-5D source/baseline/producer/comparator, `ScaledRKF45`, Cstar,
> and ADR-0015 physics are unchanged.
...
| Phase-6 checkpoint-output layer | **NOT IMPLEMENTED / ADR-0017 PROPOSED — OWNER RATIFICATION REQUIRED** | The proposed Phase-6-only owner would keep the adaptive RKF45 main integration uninterrupted, treat observation schedules as passive metadata, return accepted endpoints exactly, and reconstruct strict-interior checkpoints with isolated two-level self-qualified local rk8pd. The oracle evidence is self-qualified; the method is not accepted production authority. Phase-5D, `ScaledRKF45`, Cstar, R20 quadrature, and BNV physics remain unchanged. `docs/adr/ADR-0017-phase6-passive-checkpoint-reconstruction.md:1-12` |
