...
| **3E** | ◐ **ACTIVE — 3E-0, I1 and I2 COMPLETE; I3 optional, I4 REQUIRED.** Canonical TOV path | ADR-0005 **ACCEPTED**. **I1**: `Solve()` delegates to `SingleStarSolveToTOVPoints`; one radial integration per sequence member; all Path-1 values bit-identical. **I2**: Path-1 `Append` (Λ) and `FinalizeSurface` (proper volume) migrated to `CompactStar::Geometry` — Λ bit-identical, Path-1 `B` moved ≤`1.640e-16` against the unwidened `1.0e-15`, and Path-1/Path-2 `B` are now **bitwise identical**, closing the ADR-0004 gap on both ordinary paths. **I4 remains required**: migrate `GenTestSequence`, delete `RadiusLoop`. **Fail-closed #3 still DISCHARGED for the `Solve()` workflow, PARTIALLY OPEN overall.** Evidence: [`PHASE3E_I1_CANONICAL_TOV.md`](../validation/PHASE3E_I1_CANONICAL_TOV.md), [`PHASE3E_I2_PATH1_GEOMETRY.md`](../validation/PHASE3E_I2_PATH1_GEOMETRY.md) | **STRUCTURAL** (fail-closed #3) | 3B, 3D ✅ | **YES — accepted** | yes |
...
unified) — see
[`PHASE3E_I1_CANONICAL_TOV.md`](../validation/PHASE3E_I1_CANONICAL_TOV.md). **`3E-I2` ✅ COMPLETE** — Path-1 `Append` Λ and
`FinalizeSurface` proper volume now use `CompactStar::Geometry`; see
[`PHASE3E_I2_PATH1_GEOMETRY.md`](../validation/PHASE3E_I2_PATH1_GEOMETRY.md). Still pending inside
Phase 3E, which therefore remains **ACTIVE**: **I3** (optional `NStar` construction convergence —
take it only if later evidence justifies it) and **I4** (**required**: add focused coverage for
`GenTestSequence`, migrate it, delete `RadiusLoop`, and close fail-closed condition #3 fully).
