...
| **3C** unit U-2 (`k_B`) | `heat_capacity_v1`, `heat_capacity_real_star`, `passive_cooling_regression`, full 13/13 | **PREDECLARED `≤1.7e-11` — NOT MET.** The 3C audit measured **`1.3e-6`** on the passive-cooling trajectory: the RKF45 driver runs at `rtol = 1e-6`, so a `1.7e-11` shift in `C_⋆` changes adaptive step selection and the output lands at the integrator's tolerance scale, not the input scale. `L_ν` is analytically independent of `k_B` yet moves `1.01e-6`, which confirms the mechanism. The predeclaration was derived from the constants alone and did not anticipate this. **It is left unchanged here rather than widened after the fact**; the gap is an owner-adjudication item, not a tolerance to be re-chosen. |
...
| **3C** | ◐ **AUTHORITY AUDIT COMPLETE — AWAITING OWNER ADJUDICATION.** Implementation **not** started. Unify `k_B` precision | Owner: **ZakiLib**, which already publishes `K_BOLTZ_EV` (`3e-16` from SI-exact) and whose symbol CompactStar already links — **no ZakiLib change, no archive rebuild**. GSL rejected: pre-2019, `1.01e-6` from exact. **No option is bit-identical.** Measured trajectory impact **`1.3e-6`**, ~10⁵× the `1.7e-11` predeclared below, via adaptive-stepper amplification — so the goldens would need re-baselining. Evidence: [`PHASE3C_BOLTZMANN_AUTHORITY.md`](../validation/PHASE3C_BOLTZMANN_AUTHORITY.md) | **numerical-method / constant-authority** (not engineering) | 3A ✅ | no | **yes** |
