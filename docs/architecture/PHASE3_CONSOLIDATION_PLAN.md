...
| **3E** | ◐ **ACTIVE — ADR-0005 ACCEPTED; 3E-0 and 3E-I1 COMPLETE; I2/I3/I4 PENDING.** Canonical TOV path | 3E-0 measured the two paths **bit-identical** in all 25 radial columns (H2 verified). [`ADR-0005`](../adr/ADR-0005-canonical-tov-numerical-primitive.md) (**ACCEPTED 2026-09-02**): Q1 retain `Solve()` subordinate, Q2 preserve `_Sequence.tsv`, Q3 = **P3 staged**, Q4 preserve hooks. **3E-I1 landed**: `Solve()` delegates to `SingleStarSolveToTOVPoints`, one radial integration per sequence member, all Path-1 values **bit-identical** (equivalence artifact byte-identical), `_Sequence.tsv` contract now guarded. `RadiusLoop` **retained and marked non-authoritative** — `GenTestSequence` (public, unexercised) still calls it, so deletion is I4. **Fail-closed #3 DISCHARGED for the visible-sector `Solve()` workflow; PARTIALLY OPEN overall.** Evidence: [`PHASE3E_I1_CANONICAL_TOV.md`](../validation/PHASE3E_I1_CANONICAL_TOV.md) | **STRUCTURAL** (fail-closed #3) | 3B, 3D ✅ | **YES — accepted** | yes |
...
unified) — see
[`PHASE3E_I1_CANONICAL_TOV.md`](../validation/PHASE3E_I1_CANONICAL_TOV.md). Still pending inside
Phase 3E, which therefore remains **ACTIVE**: **I2** (Path-1 ADR-0004 proper-volume conformance
under the unspent `1.0e-15` bound), **I3** (optional `NStar` construction convergence), and
**I4** (migrate `GenTestSequence`, delete `RadiusLoop`, and close fail-closed condition #3 fully).
