...
The next increment is **3E-0**: build a behavior-measurement/equivalence harness for the **two
live TOV paths** *before* proposing or implementing canonical-TOV ownership. 3D deliberately left
TOV Path 1's inline proper-volume forms unmigrated for exactly this reason — they are governed by
ADR-0004 but have no coverage that would detect a regression, and `TOVSolver::Solve` has three
live callers (`spin_therm_evol_main`, `tov_debug_main`, `sig_omega`). **Phase 3 is not complete.** **That ADR is now drafted:**
